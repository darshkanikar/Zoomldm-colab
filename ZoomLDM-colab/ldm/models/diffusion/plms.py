"""
PLMS Sampler (Colab Compatible)
-------------------------------
Patched for:
- PyTorch ≥ 2.1 (Colab default)
- Python 3.12+
- Single GPU (no DDP)
- Optional autocast for faster inference
"""
...
# Safe imports for Lightning utilities (optional)
try:
    ...
    try:
        ...
        def rank_zero_only(fn):  # fallback
            return fn
...
class PLMSSampler(object):
    ...
    # Helper for registering buffers on correct device
    ...
    def sample(
        ...
        **kwargs,
        ...
        """Main entry point for PLMS sampling."""

        if conditioning is not None:
            if isinstance(conditioning, dict):
                cbs = conditioning[list(conditioning.keys())[0]].shape[0]
                if cbs != batch_size:
                    print(f"⚠️ Warning: Got {cbs} conditionings but batch-size is {batch_size}")
            else:
                if conditioning.shape[0] != batch_size:
                    print(f"⚠️ Warning: Got {conditioning.shape[0]} conditionings but batch-size is {batch_size}")
        ...
        if verbose:
            print(f"🧩 Data shape for PLMS sampling: {size}")
    ...
    def plms_sampling(
        ...
        iterator = tqdm(time_range, desc="PLMS Sampler", total=total_steps) if verbose else time_range
        old_eps = []
...
if __name__ == "__main__":
    print("✅ plms_colab.py loaded successfully")
